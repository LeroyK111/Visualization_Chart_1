...
function newRow(params) {
  var rowData = [];
  for (let index = 0; index < 7; index++) {
    rowData.push(Math.floor(Math.random() * 1500));
    if (index == 6) return rowData;
  }
}
...
export function update2() {
  randomData = +(Math.random() * 60).toFixed(2);
  for (const iterator of option_right2.series) {
    iterator.data[0].value = randomData;
  }
}

// 天气表盘
export var option_right2 = {
  series: [
    {
      type: "gauge",
      center: ["50%", "60%"],
